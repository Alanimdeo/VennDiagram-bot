...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (guildQueue) {
            ...
            await interaction.editReply({ embeds: [new MessageEmbed().setColor('#008000').setTitle(`:${emoji}: 반복 ${interaction.options.getInteger('설정') == 0 ? '해제' : '설정'}됨`).setDescription(`반복이 ${interaction.options.getInteger('설정') == 0 ? '해제' : '설정'}되었습니다.${interaction.options.getInteger('설정') > 0 ? `\n현재 설정: ${interaction.options.getInteger('설정') == 1 ? '곡' : '목록'}` : ''}`)]});
