...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option: any) => option.setName("시간").setDescription("이동할 시간을 입력하세요.").setRequired(true)),
    async execute(interaction: any) {
        ...
        if (guildQueue) {
            ...
            if (!/^[0-9:.]+$/.test(interaction.options.getString("시간"))) {
                ...
                if (input.length == 3) seekTime = Number(input[0]) * 3600000 + Number(input[1]) * 60000 + Number(input[2]) * 1000;
                else seekTime = Number(input[0]) * 60000 + Number(input[1]) * 1000;
            } else {
