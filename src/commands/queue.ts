...
module.exports = {
    data: new SlashCommandBuilder().setName("목록").setDescription("재생 목록을 확인합니다."),
    async execute(interaction: any) {
        await interaction.deferReply();
        let guildQueue = interaction.client.player.getQueue(interaction.guildId);
        if (guildQueue) {
            let message = ":scroll: 재생 목록\n";
            guildQueue.songs.forEach((song: typeof Song, index: number, array: Array<any>) => {
            ...
            await interaction.editReply({
                embeds: [new MessageEmbed().setColor("#ff0000").setTitle(":warning: 오류").setDescription("재생 목록이 없습니다.")],
            });
        }
    },
};
